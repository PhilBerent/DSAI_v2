...
For each entity, include its name, alternate names and a description . If the entity is refered to by only one name in the block then the "alternate_names" field should be blank. The 'description' field should be a brief description of any information that is learned about the entity in the block. This should be purely descriptive information and should not include page references. For example it might become apparent in a block that Mr Brown lives in a house in Buckinghamshire and that his wife's name is Jane. The description for "Mr Brown" for that block then might read "Mr Brown lives in Buckinghamshire and is married to Jane" a description for the location "32 Aubert Park" might read "The home of Mr and Mrs Jones" the desciption for the organization "The Odeon Cinema" might read "Cinema located on Holloway Road" and so on. If nothing is learned about the entity in the block then the description should be blank. The 'key_entities_in_block' field should be a dictionary with keys 'characters', 'locations', and 'organizations', each containing a list of entities with their details. 
If an entity (character, location or organization) is refered to by more than one name in the block then only one entry should be made for that entity in the list. The name of the entity should be the name that gives the fullest description of the entity which might be one of the names given or even a combination of the names given. The other ways in which the entity is refered to should be listed in 'alternate_names'. For example if a character is refered to as "Mr Brown" and "Philip" in the block then the best name for the character would be "Mr Philip Brown" and 'alternate_names' would contain "Mr Brown" and "Philip". If an entity is refered to by more than one name in a block then the description field should be a summary of what is learned about the entity in that block. For example the description might read "Mr Brown (also known as Philip) lives in Buckinghamshire and is married to Jane". Similarly, if the organization Pizza Hut is also refered to as "the Pizza Joint" in the block then the best name for the organization would  be "Pizza Hut" and 'alternate_names' would contain "the Pizza Joint". The description field might read "Pizza Hut (also known as the Pizza Joint) is located on Holloway Road".
