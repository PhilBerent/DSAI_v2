...
def get_anal_large_block_promptOld(block_data: Dict[str, Any], additional_data: Any = None) -> str:
    ...
    prompt = f"""
Analyze the following large text block from a document. Extract a concise summary, key entities primarily featured *in this block*, and identify any structural marker (like Chapter/Part title) found near the beginning of this block. 
...
The 'key_entities_in_block' field should be a dictionary with keys 'characters', 'locations', and 'organizations', each containing a list of entities which are contained in the block with their details.
...
For example if a character is refered to as "Mr Brown" and "Philip" and "Phil" in the block, then the best name for the character would be "Mr Philip Brown" because it provides the fullest description, and so "Mr Philip Brown" would be in the 'name' field, and 'alternate_names' would contain "Mr Brown" and "Philip", and "Phil". If the first name and last name are known then they should be used in the 'name' field. This should be preceded by the appropriate prefix ("Mr.", "Mrs.", "Miss.", "Ms.", "Doctor", etc.) if this is known. Character names should consist only of "name elements" such as a first name or last name or a nickname". Names should not include additional descriptors. So "Mr William Jones" is a good character name whereas "the kind Bobby Williams" or "old Mrs Jones" are not as they include additional descriptors. Names should not include "the" or "a" or "an" at the start of the name. For example "the Jones Family" should be "Jones Family", "a house" should be "house", "The Richmond Football Team" should be "Richmond Football Team" and so on
I an entry is refered to by more than one name in the block, only one entry should be made for that entity in the list character list, location list or organization list. The entry will have the most complete name in the 'name' field, and the other names in the 'alternate_names' field.
...
If an entity is refered to by more than one name in a block then the description field should be a summary of what is learned about the entity in that block. For example the description might read "Mr Brown (also known as Philip) lives in Buckinghamshire and is married to Jane". Similarly, if the organization Pizza Hut is also refered to as "the Pizza Joint" in the block then the best name for the organization would  be "Pizza Hut" and 'alternate_names' would contain "the Pizza Joint". The description field might read "Pizza Hut (also known as the Pizza Joint) is located on Holloway Road".
